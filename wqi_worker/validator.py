...
def validate_river_marker(marker: dict, param_config: dict) -> list[str]:
    errors    = []
    river_id  = marker.get("river_id")
    lat       = marker.get("lat")
    lng       = marker.get("lng")
    params    = marker.get("parameters") or {}

    if not river_id:
        errors.append("missing river_id")

    if lat is None:
        errors.append(f"river_id={river_id}: missing lat")
    else:
        try:
            if not (-90.0 <= float(lat) <= 90.0):
                errors.append(f"river_id={river_id}: lat {lat} out of range")
        except (TypeError, ValueError):
            errors.append(f"river_id={river_id}: lat '{lat}' is not numeric")

    if lng is None:
        errors.append(f"river_id={river_id}: missing lng")
    else:
        try:
            if not (-180.0 <= float(lng) <= 180.0):
                errors.append(f"river_id={river_id}: lng {lng} out of range")
        except (TypeError, ValueError):
            errors.append(f"river_id={river_id}: lng '{lng}' is not numeric")

    if not isinstance(params, dict) or len(params) == 0:
        errors.append(f"river_id={river_id}: parameters must be a non-empty object")
        return errors
    ...
    if not known:
        errors.append(
            f"river_id={river_id}: none of the submitted parameters "
            f"({list(params.keys())}) are defined in Parameters.xlsx"
        )
